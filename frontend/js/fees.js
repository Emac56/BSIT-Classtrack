  ...
  const rowsHTML = rows.map(r => {
    ...
    return `<tr id="fee-row-${r.student_id}-${r.fee_type_id}">
      <td>
        ...
        ${gBadge ? `<span style="margin-left:5px;font-size:.75rem;color:${gColor};font-weight:700">${gBadge}</span>` : ''}
      </td>
      <td><code>${esc(r.student_number || '')}</code></td>
      <td>${esc(r.fee_name)}</td>
      <td>${amountDisplay}</td>
      <td>
        <span class="badge badge-${isPaid ? 'paid' : 'unpaid'}">${isPaid ? '✅ PAID' : '❌ UNPAID'}</span>
        ${overdue ? '<br><span class="overdue" style="font-size:.7rem">OVERDUE</span>' : ''}
      </td>
...
// Quick one-tap mark as paid (full amount, today's date)
async function markPaidRow(studentId, feeTypeId, studentName, feeName, feeAmount) {
  ...
  try {
    await api.recordPayment({
      student_id: studentId, fee_type_id: feeTypeId,
      amount_paid: feeAmount, payment_date: todayISO(),
      notes: '', _user: { id: user.id, name: user.full_name },
...
// Edit payment — shows amount, date, notes + live change calculator
function openEditPayment(studentId, feeTypeId, studentName, feeName, feeAmount, currentPaid, currentDate) {
  ...
  // Trigger change calculator on open
...
function calcChange(feeAmount) {
  ...
  if (given <= 0) {
    ...
    box.innerHTML = `💵 Change to give back: <span style="font-size:1.1rem">${peso(change)}</span>`;
  } else if (change < 0) {
    box.style.display = 'block';
    box.style.background = '#fef3c7';
    box.style.color = '#92400e';
    box.innerHTML = `⚠️ Short by: <span style="font-size:1.1rem">${peso(Math.abs(change))}</span> (partial payment)`;
  } else {
    box.style.display = 'block';
    box.style.background = '#d1fae5';
    box.style.color = '#065f46';
    box.innerHTML = `✅ Exact payment — no change`;
  }
}
...
async function saveEditPayment(studentId, feeTypeId, studentName, feeAmount) {
  ...
  try {
    await api.recordPayment({
      student_id: studentId, fee_type_id: feeTypeId,
      amount_paid: amount, payment_date: date || todayISO(),
      notes, _user: { id: user.id, name: user.full_name },
      section_id: user.section_id
    });
...
function exportFeesPDF() {
  ...
  exportPDF('Fees & Payments — ' + (window.currentSectionName || ''),
    ...
    rows.map(r => [`${r.last_name}, ${r.first_name}`, r.student_number||'', r._gender||'',
      r.fee_name, peso(r.fee_amount), r.status==='paid'?peso(r.amount_paid):'—',
      r.status.toUpperCase(), r.payment_date?fmtDate(r.payment_date):'—']),
    'fees.pdf');
}
...
function exportFeesXLSX() {
  ...
  exportExcel('Fees',
    ...
    rows.map(r => [`${r.last_name}, ${r.first_name}`, r.student_number||'', r._gender||'',
      r.fee_name, r.fee_amount, r.amount_paid||'', r.status, r.payment_date||'']),
    'fees.xlsx');
}
