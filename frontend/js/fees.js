        ...
        <div class="ai-panel-header">
          <div style="display:flex;align-items:center;gap:10px">
            ...
            <div>
              ...
              <div style="font-size:.65rem;color:rgba(255,255,255,.5)">Powered by Claude AI · Real-time data</div>
            </div>
          </div>
        ...
        <div id="aiMessages" class="ai-messages">
          <div class="ai-msg ai-msg-bot">
            <div class="ai-bubble">
              ...
              Itanong mo lahat tungkol sa bayad ng inyong section — Taglish ok!
              <div class="ai-suggestions">
                ...
                <button onclick="askAI('Sino ang may change na ibinalik?')">Sino may change?</button>
                <button onclick="askAI('Magkano pa ang kulang sa collection?')">Collection status</button>
              </div>
            </div>
          </div>
        </div>
        <div class="ai-input-row">
          <input type="text" id="aiChatInput" class="ai-input"
            placeholder="e.g. Si Jimenez bayad na ba? May change?"
            maxlength="500"
...
function buildFeeContext() {
  ...
  types.forEach(ft => {
    byType[ft.id] = {
      ...
      paid: [], unpaid: [], withChange: [], partial: []
    };
  });
  ...
  status.forEach(r => {
    ...
    if (!grp) return;
    const name   = `${r.last_name}, ${r.first_name.split(' ')[0]}`;
    const amtPd  = parseFloat(r.amount_paid || 0);
    const fee    = parseFloat(r.fee_amount  || 0);
    const change = amtPd - fee;
    if (r.status === 'paid') {
      grp.paid.push({ name, amtPd, fee, change, date: r.payment_date });
      if (change > 0) grp.withChange.push({ name, change });
      if (change < 0) grp.partial.push({ name, short: Math.abs(change) });
    } else {
      const overdue = r.due_date && r.due_date < today;
      grp.unpaid.push({ name, fee, overdue });
    }
  });

  const dateStr = new Date().toLocaleDateString('en-PH', { weekday:'long', year:'numeric', month:'long', day:'numeric' });
  let ctx = `You are an AI payment assistant for a Filipino college section using ClassTrack. Today is ${dateStr}.\n\n`;
  ctx += `CURRENT PAYMENT DATA:\n`;

  for (const grp of Object.values(byType)) {
    const total = grp.paid.length + grp.unpaid.length;
    ctx += `\n📌 ${grp.name} — ₱${grp.amount.toFixed(2)}/student | ${grp.paid.length}/${total} paid\n`;
    if (grp.paid.length)
      ctx += `  ✅ PAID: ${grp.paid.map(p => p.name + (p.change > 0 ? ` [change ₱${p.change.toFixed(2)}]` : p.change < 0 ? ` [short ₱${Math.abs(p.change).toFixed(2)}]` : ' [exact]')).join(' | ')}\n`;
    if (grp.unpaid.length)
      ctx += `  ❌ UNPAID: ${grp.unpaid.map(u => u.name + (u.overdue ? ' [OVERDUE]' : '')).join(' | ')}\n`;
    if (grp.withChange.length)
      ctx += `  💵 WITH CHANGE: ${grp.withChange.map(x => `${x.name} ₱${x.change.toFixed(2)}`).join(' | ')}\n`;
  }

  ctx += `\nRULES:\n`;
  ctx += `- Respond in Taglish (Filipino + English mix) — friendly tone like a classmate.\n`;
  ctx += `- Use bullet points for lists of names. Be concise and scannable.\n`;
  ctx += `- Use ✅ for paid, ❌ for unpaid, 💵 for change, ⚠️ for overdue.\n`;
  ctx += `- If asked about a specific student by last name, find them in the data and answer directly.\n`;
  ctx += `- Include peso amounts when relevant (e.g. "may change na ₱50").\n`;
  ctx += `- Keep responses short — max 200 words unless a full list is needed.\n`;
  return ctx;
}
